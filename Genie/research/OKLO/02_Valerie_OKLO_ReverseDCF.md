...
- The **bear scenario ($6.98B)** at 16% CoE is demanding: Oklo would need to be running roughly 85+ commercial reactors (at 75 MWe each, ~90% capacity factor, $120/MWh PPA) to generate $6.98B in power revenue. That implies ~6.4 GW of installed capacity — achievable only if the full Switch MPA (12 GW) is substantially built out.
- The **base scenario ($4.43B)** at 14% CoE implies ~54 reactors (~4 GW) — still a large fleet deployment over 8–12 years.
- The **bull scenario ($2.90B)** at 12% CoE implies ~35 reactors (~2.6 GW) — more plausible, though still requires successful fleet-scale licensing and construction.
...
- **30% probability for Bear scenario:** Reaching $6.98B requires near-flawless execution across 85+ reactor deployments over a decade. This probability accounts for NRC licensing risk (one prior denial), HALEU supply chain constraints, construction FOAK risk, and competition from TerraPower, Kairos, and next-generation energy storage. Only large-scale technology platform companies with proven scale have successfully executed comparable ramps.

- **50% probability for Base scenario:** $4.43B (~54 reactors, ~2030–2034 timeframe) is more achievable if Aurora-INL succeeds, Ohio Phase 1 proceeds on schedule, and the Part 57 regulatory framework is adopted. The 50% reflects that the technology works (demonstrated at lab scale), regulatory momentum is building, but commercial execution at scale has zero precedent.

- **75% probability for Bull scenario:** $2.90B (~35 reactors by 2032–2034) is the most achievable outcome — consistent with completing the Meta/Ohio first phase and Aurora-INL plus a modest additional fleet. This probability reflects genuine optimism about the 2027 INL operation target and the accelerated NRC review precedent. The 25% failure probability reflects the non-zero chance of a catastrophic single event (NRC denial, construction failure, financing freeze) that derails even this modest outcome.
...
| 2027 | 15 MWe (Aurora-INL pilot) | ~$15–18M | Aurora-INL FCO Q4 2027 |
...
| 2029 | 15 MWe | ~$13M | Aurora-INL FCO |
...
| 2031 | 15 MWe | ~$10M | Aurora-INL FCO (minimal scale) |
...
| Share Count (Dec 2025) | ~157M | — |
| Share Count (June 2026) | ~174M | — |
| Implied 6-Month Dilution | +10.8% | High |
...
- **Aurora-INL (15 MWe pilot):** Target construction cost ~$60M. But this is a demonstration unit at a DOE facility with unique access. Commercial units will cost more.
...
The base implied revenue ($4.43B steady-state) requires roughly 54 commercial reactors operating at scale — each having been licensed, constructed, and commissioned without catastrophic setbacks. For context:
...
1. At an EV of $8.06B and using the bull scenario cost of equity (12%) and FCF margin (25%), the implied steady-state revenue is $2.90B — achievable only if ~35 commercial reactors are operating (roughly matching the Meta Ohio campus at full build-out plus Aurora-INL). This is an aggressive but not impossible 2032–2034 outcome.
...
- Scaling from 1 to 54 reactors while maintaining cost discipline: 9/10
